...
# test_province_lookup_fixed.py
from pyspark.sql import SparkSession, functions as F, Row
from sedona.register import SedonaRegistrator

spark = (
    SparkSession.builder
    .appName("ProvinceLookup")
    .config(
        "spark.jars",
...
# 1) Load GeoJSON (multiline)
raw = spark.read.option("multiline", "true").json("etl_pipeline/data/geo/gadm41_JPN_1.json")

# 2) Explode features
features = raw.select(F.explode("features").alias("feature"))

# 3) Chuyển geometry (struct) -> JSON string, rồi thành geometry Sedona
provinces = (
    features
...
provinces.createOrReplaceTempView("japan_provinces")

print("✅ Provinces loaded:", provinces.count())

# 4) Hàm lookup 1 điểm
def find_province(lon: float, lat: float):
    point_df = spark.createDataFrame([Row(lon=float(lon), lat=float(lat))])
    # ST_Point yêu cầu double; cast đảm bảo đúng kiểu
    point_df = point_df.withColumn("point", F.expr("ST_Point(double(lon), double(lat))"))
    point_df.createOrReplaceTempView("query_point")

    row = spark.sql("""
        SELECT p.province_name
        FROM japan_provinces p, query_point q
        WHERE ST_Contains(p.geometry, q.point)
    """).first()
    return row.province_name if row else None

# 5) Test
tests = [
    (139.6917, 35.6895),  # Tokyo
    (135.5023, 34.6937),  # Osaka
    (132.5553, 32.7478),  # Kochi (ví dụ bạn đưa)
]
for lon, lat in tests:
    print(f"({lon}, {lat}) -> {find_province(lon, lat)}")
